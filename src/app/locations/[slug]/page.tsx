...
export default async function CityPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {/* Hero */}
      <section className="bg-surface-raised text-text-primary py-12">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            ...
            {vendors.length === 0 ? (
              <Card padding="lg" className="text-center">
                <p className="text-text-muted mb-4">No contractors listed in {city.name} yet.</p>
                <Link
                  href="/search"
            ...
            {city.description && (
              <div className="mt-8">
                <Card padding="lg">
                  ...
                  <div className="prose text-text-secondary" dangerouslySetInnerHTML={{ __html: city.description }} />
                </Card>
              </div>
            )}
          </div>
          ...
          <aside className="w-full lg:w-80 flex-shrink-0">
            <div className="sticky top-20 space-y-6">
              <Card padding="md">
                <h3 className="font-semibold text-text-primary mb-3">Quick Facts</h3>
                <dl className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <dt className="text-text-muted">City</dt>
                    <dd className="font-medium text-text-primary">{city.name}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-text-muted">State</dt>
                    <dd className="font-medium text-text-primary">{city.state}</dd>
                  </div>
                  {city.population > 0 && (
                    <div className="flex justify-between">
                      <dt className="text-text-muted">Population</dt>
                      <dd className="font-medium text-text-primary">{city.population.toLocaleString()}</dd>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <dt className="text-text-muted">Contractors</dt>
                    <dd className="font-medium text-text-primary">{vendors.length}</dd>
                  </div>
                </dl>
              </Card>

              <Card padding="md" className="bg-neon-subtle border-neon-100">
                <h3 className="font-semibold text-text-primary mb-2">Get Free Quotes</h3>
              ...
              <Card padding="md">
                <h3 className="font-semibold text-text-primary mb-2">Cost Estimator</h3>
                <p className="text-sm text-text-secondary mb-3">
                  Get an instant price range for your sports surface project.
                </p>
                <Link
                  href="/cost-estimator"
                  className="block text-center border border-neon text-neon py-2 rounded-lg text-sm font-semibold hover:bg-neon-subtle"
                >
                  Estimate Costs
                </Link>
              </Card>
